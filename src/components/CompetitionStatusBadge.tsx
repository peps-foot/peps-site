...
export default function CompetitionStatusBadge({ competitionId, mode }: Props) {
  ...
  const className = useMemo(() => {
    const box = "w-32 h-9 grid place-items-center rounded border font-medium text-center";
    if (label === null) return `${box} border-gray-300 bg-gray-100 text-transparent`;
    if (color === "green") return `${box} border-green-700 text-green-800 bg-green-50`;
    if (color === "blue") return `${box} border-blue-700 text-blue-800 bg-blue-50`;
    return `${box} border-gray-500 text-gray-700 bg-gray-100`;
  }, [label, color]);

  if (label === null) return <div className="w-32 h-9 grid place-items-center rounded border border-gray-300 bg-gray-100 animate-pulse" />;
